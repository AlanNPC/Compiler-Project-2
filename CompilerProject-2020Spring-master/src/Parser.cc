...
void Parser::parse(string name,vector<std::string>& ins,string outs,string data_typeStr,string kernelStr){
    string noEmpty="";
...
vector<string> Parser::getIndexName(string str) {
    int record[] = {-1, -1, -1, -1};
...
void Parser::processOne(string code){
    ...
    isLeft = 0;
    Expr retOperand=Var::make(data_type,"ret",leftIndexList, leftShapeList);
    Stmt moveStmtAdd=Move::make(retOperand,Binary::make(
            data_type,
            BinaryOpType::Add,
            retOperand,
            Var::make(data_type,"tmp",leftIndexList, leftShapeList)),
            MoveType::MemToMem
    );
    Stmt moveStmtSub=Move::make(retOperand,Binary::make(
            data_type,
            BinaryOpType::Sub,
            retOperand,
            Var::make(data_type,"tmp",leftIndexList, leftShapeList)),
    ...
    Stmt clearTmp= Move::make(
            Var::make(data_type,"tmp",leftIndexList, leftShapeList),
            Expr(0),
            MoveType::MemToMem
    );
    main_stmt.push_back(Move::make(retOperand,Expr(0),MoveType::MemToMem));//ret[i][j]=0
    for(int i=0,size=rightExpr.size();i<size;++i){
        main_stmt.push_back(clearTmp);
        Stmt nextStmt=build_loop(rightExpr[i]);
        if(nextStmt.defined()) main_stmt.push_back(nextStmt); //for(k) tmp[i][j]=...
        used.clear();
        localRestriction.clear();
        if(i==0) {
            main_stmt.push_back(moveStmtAdd); // ret[i][j]=ret[i][j]+tmp[i][j]
        }
        else{
            if (rightOps[i - 1] == "+")
                main_stmt.push_back(moveStmtAdd);
            else
                main_stmt.push_back(moveStmtSub);
        }
    }
    if(tmp.size()>=2)
        main_stmt.push_back(Move::make(leftOperand,retOperand,MoveType::MemToMem)); //outs[i][j]=ret[i][j]
    finalLoop.push_back(LoopNest::make(leftIndexListNoRepeat, {main_stmt}));
    clear();
    if(debug) cout<<"Out of processOne"<<endl;
}

void Parser::buildConstriction(string str){
    if(debug)
...
vector<string> Parser::split(string str,const vector<char> delim){
    vector<string> ret;
    int size=str.size();
    int beg=0;
...
bool Parser::includeMathOp(string expr){
    char mathOp[]={'+','-','*','/','%'};
    int size=expr.size();
    for(int i=0;i<size;++i) {
...
map<string,int> Parser::registerIndex(string str){
    if(debug)
            ...
            if (dom.size() != indexName.size()) {
                ...
                for (int i = 0,tmpSize = dom.size(); i < tmpSize; ++i) {
                    ...
                    if(name2range.find(indexName[i])!=name2range.end()){
                        if(name2range[indexName[i]]<scale)// choose the bigger
...
Stmt Parser::build_loop(string expr){
    ...
    return LoopNest::make(loopIndex, {moveStmt});
}
...
Stmt Parser::build_if_stmt(Stmt trueExpr){
    if(debug)
...
Expr Parser::build_cond(int ind){
...
Expr Parser::parse_AddSub(string expr) {
...
Expr Parser::parse_Mod(string expr){
...
Expr Parser::parse_MulDiv(string term){
...
Expr Parser::parse_operand(string operand){
    ...
    if(record[0]==-1||record[1]==-1){//const
        if(operand[beg]>='0'&&operand[beg]<='9') { //number
            ...
            cout<<"the index name:_"<<name<<"_ not found!"<<endl;
        ...
        varMP.insert(make_pair(name,value));
        if(debug) cout<<"Out of parse_operand..."<<endl;
        if (!isLeft)// add select to any var on the right
        {
            for (int i = 0; i < shape.size(); ++i)
            {
                value = Select::make(data_type, Compare::make(data_type,CompareOpType::GE,indexExpr[i],Expr(0)), value, Expr(0));
                value = Select::make(data_type, Compare::make(data_type,CompareOpType::LT,indexExpr[i],Expr(shape[i])), value, Expr(0));
            }
        }
        return value;
    }
